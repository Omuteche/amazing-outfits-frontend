...
export function ProductCard({ product, onWishlistToggle, isInWishlist }: ProductCardProps) {
  ...
  const mainImage = resolveImageUrl(product.images?.[0] || '/placeholder.svg');
  ...
  return (
    <div
      ref={cardRef}
      className="group glass-card rounded-lg overflow-hidden hover-lift transition-transform duration-300 ease-out"
      style={tiltStyle}
    ...
    >
          ...
          >
            <Heart
              className={`h-5 w-5 transition-colors ${
                isInWishlist ? 'fill-accent text-accent' : 'text-foreground'
              }`}
            />
          </Button>
        )}
      ...
      <div className="p-4">
        {product.brands && (
          <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1">{product.brands.name}</p>
        )}
        ...
        {product.sizes && product.sizes.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-1">
            {product.sizes.slice(0, 5).map((size) => (
              <span key={size} className="text-xs px-2 py-1 bg-secondary rounded text-muted-foreground">
                {size}
              </span>
            ))}
            {product.sizes.length > 5 && (
              <span className="text-xs px-2 py-1 text-muted-foreground">+{product.sizes.length - 5}</span>
            )}
          </div>
        )}
      </div>
